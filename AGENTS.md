...
- Browser-runtime code map:
  ...
  - Startup cancellation must flow through the whole bridge path. `ChromeTtsBridge.speak()` and `ChromeTtsBridge.preload_voice()` must pass their request/preload `cancelEvent` into `ChromeTtsBridge.ensure_connection(cancelEvent=...)`; `ensure_connection()` must pass it into `BrowserProcessManager.start_and_get_websocket_url()`, `WasmTtsEngineBridge.enable_cdp_domains()`, and `WasmTtsEngineBridge.wait_until_ready()`. `WasmTtsEngineBridge.enable_cdp_domains()` must pass the same event into each `CdpClient.request()`.
  - Runtime health and soft recycle live in `bridge.py`: `RUNTIME_MEMORY_STARTUP_GRACE_SECONDS`, `RUNTIME_MEMORY_CHECK_INTERVAL_SECONDS`, `RUNTIME_PRIVATE_BYTES_RECYCLE_THRESHOLD`, `RUNTIME_WORKING_SET_BYTES_RECYCLE_THRESHOLD`, `RUNTIME_MEMORY_RECYCLE_CONFIRMATIONS`, `_runtime_error_requires_recycle()`, `_process_tree_memory_usage()`, `BrowserProcessManager.browser_memory_usage()`, `WasmTtsEngineBridge.runtime_busy`, `ChromeTtsBridge._mark_runtime_error_for_recycle()`, `ChromeTtsBridge._mark_memory_recycle_if_needed_locked()`, and `ChromeTtsBridge.maybe_recycle_runtime()`. Runtime/CDP timeout or closed-WebSocket failures should mark the Chromium runtime for urgent recycle after the current speech request; memory threshold failures should ignore normal Chromium/WASM cold-start spikes, require confirmed high-memory samples after the startup grace/interval, and recycle only when the synth worker reports an idle queue so speech is not cut off mid-utterance.
  - Synth-side recycle scheduling lives in `SynthDriver._maybe_recycle_bridge_after_request()`. Call it after each non-cancelled speech request so unhealthy runtime state can be cleared automatically, but keep browser termination out of NVDA's main thread and out of active audio callbacks.
  ...
  - `settings.py` runtime status controls must use focusable read-only text sized through `bind_read_only_text_focus_announcement()` so the selected/effective Chromium runtime message is reachable by Tab and reviewable without delayed automatic re-announcements. Changing the runtime choice should refresh the status preview immediately from a runtime snapshot, but the saved runtime must still change only through Settings OK/Apply; if save is rejected, reselect the saved runtime and refresh the status for that saved value.
  ...
  - Browser profile startup fallback lives in `BrowserProcessManager.start_browser()`, `BrowserProcessManager.start_and_get_websocket_url()`, `BrowserProcessManager._browser_choices_or_raise()`, `BrowserProcessManager._start_browser_choice()`, `BrowserProcessManager._start_first_available_browser()`, `BrowserProcessManager._read_devtools_port()`, `_BrowserProfileInUseError`, `_browser_profile_in_use_error()`, `_get_browser_profile_dir()`, `_cleanup_old_browser_profiles()`, `_release_chrome_profile()`, and `_remove_chrome_profile()`. Start with the persistent `persistentSession` profile so Chromium can reuse WASM/code cache; if Chromium exits with profile-in-use code 21, retry once with a temporary `session-<pid>-<timestamp>` profile under the same runtime profile root before trying the next runtime. `BrowserProcessManager._get_browser_profile_dir()` and `_cleanup_old_browser_profiles()` must check the startup cancel event before profile cleanup work and inside recursive persistent-profile scans, so synth switches do not wait for stale filesystem cleanup. `BrowserProcessManager._read_devtools_port()` must treat a just-created `DevToolsActivePort` file as temporarily unavailable when Windows/Chromium has not finished writing or releasing it: retry on `PermissionError`, `OSError`, empty content, invalid text, or out-of-range ports while the browser process remains alive. This retry is shared by Google Chrome, Microsoft Edge, and Brave; do not add Edge-only handling unless the browser behavior truly diverges.
...
- Automatic language profile code map:
  ...
  - Automatic language profile value controls are shown and hidden inside the automatic language static-box sizer. Preserve group-local layout item tracking in `_refresh_auto_language_profile_value_controls()` so expanding preferred/profile controls updates that group without causing the following Updates group to overlap in NVDA's settings panel.
  ...
  - Language detection wrapper code lives in `language_detector.py`: `_DLL_DIR`, `_DLL_NAMES`, `_LANGUAGE_ALIASES`, `_CHINESE_LANGUAGE_ROOTS`, `DetectionResult`, `_Cld2Detector.detect()`, `_Cld2Detector._load_library()`, `detect_language()`, `_candidate_for_language()`, `language_match_keys()`, `_language_aliases()`, `_language_family()`, `_language_root()`, and `_normalize_language()`. Keep x86/x64 DLL selection compatible with the running NVDA/Python architecture and keep `cld2.dll` as a compatibility fallback after the architecture-specific DLL name.
...
- Volatile speech cache code map:
  ...
  - Runtime recycle cleanup lives in `SynthDriver._maybe_recycle_bridge_after_request()` and `_clear_short_audio_cache()`. Clearing the volatile PCM cache after a Chromium/WASM runtime recycle is acceptable because it releases memory and avoids retaining audio across a runtime-health reset; do not write the cache to disk.
...
- Browser-side engine startup, package readiness, and error cleanup live in `googleTtsForNvda/synthDrivers/googleTtsForNvda/web/bridgeHarness.js`: `isTtsEngineInstance()`, `getTtsEngine()`, `ensureEngineInitialized()`, `ensureLanguageReady()`, `stopActiveSynthesis()`, `googleTtsForNvdaPreload()`, and `googleTtsForNvdaSpeak()`. `googleTtsForNvdaSpeak()` must call `stopActiveSynthesis()` on both successful completion and speech errors so audio buffers, timers, session tokens, and engine `onStop()` state do not leak into the next request.
...
- Preload code map:
  - `_warm_current_voice_async()` starts the cancellable `googleTtsForNvda.preload` thread, optionally waits for the short Voice/Variant-change debounce, ensures the browser/CDP bridge is connected with the preload cancel event, treats `CdpCancelled` as normal cancellation, and runs only the priority preload list.
